...
document.getElementById('username').addEventListener('input', pickHandler)
...
function autocomplete(inp, arr) {
	...
	inp.addEventListener("input", function(e) {
		...
		for (i = 0; i < arr.length; i++) {
		  ...
		  if (arr[i].substr(0, val.length).toUpperCase().replace("Ü","U") == val.toUpperCase().replace("Ü","U")) {
 ...
 var places_dict={}
 var lugares=[]
//  places.map(x=>x['nombre'])
 places.forEach(element=>{places_dict[element["nombre"]]=element["clave"].toString();lugares.push(element["nombre"])})
  console.log(lugares);
...
function pickHandler () {
	...
	if(reserva==true){
		...
		;
		if(ci.value.trim().length==11 && passenger.value.trim()!='' && !isNaN(ci.value.trim()) && username.value.trim()!='' && password.value.trim()!=''&& lugares.includes(pickers.origen.value) && lugares.includes(pickers.destino.value)){
			...
			var data=JSON.stringify({"username":pickers.username,"password":pickers.password,"nombre":pickers.passengername,"dni":pickers.passengerdni,"reserva":reserva,"date":pickers.date.value,"exactdate":pickers.radiovalue,"transporte":pickers.transportvalue,"origen":places_dict[pickers.origen.value],"destino":places_dict[pickers.destino.value]})
			console.log(data)
		...
		if(username.value.trim()!='' && password.value.trim()!='' && lugares.includes(pickers.origen.value) && lugares.includes(pickers.destino.value)){
			// console.log(pickers.origen.value)
			// console.log(pickers.destino.value)
			pickers.username= username.value,
			pickers.password= password.value,
